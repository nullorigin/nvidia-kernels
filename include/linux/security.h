...
int cap_inode_removexattr(struct mnt_idmap *idmap, struct dentry *dentry,
                          const char *name);
...
int security_inode_init_security(struct inode *inode, struct inode *dir,
                                 const struct qstr *qstr, initxattrs initxattrs,
                                 void *fs_data);
...
int security_inode_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
                           struct iattr *attr);
void security_inode_post_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
                                 int ia_valid);
int security_inode_getattr(const struct path *path);
int security_inode_setxattr(struct mnt_idmap *idmap, struct dentry *dentry,
                            const char *name, const void *value, size_t size,
                            int flags);
int security_inode_set_acl(struct mnt_idmap *idmap, struct dentry *dentry,
                           const char *acl_name, struct posix_acl *kacl);
void security_inode_post_set_acl(struct dentry *dentry, const char *acl_name,
                                 struct posix_acl *kacl);
int security_inode_get_acl(struct mnt_idmap *idmap, struct dentry *dentry,
                           const char *acl_name);
int security_inode_remove_acl(struct mnt_idmap *idmap, struct dentry *dentry,
                              const char *acl_name);
...
int security_inode_removexattr(struct mnt_idmap *idmap, struct dentry *dentry,
                               const char *name);
...
int lsm_fill_user_ctx(struct lsm_ctx __user *uctx, u32 *uctx_len, void *val,
 ...
 * Returns true if there is a value set, false otherwise
 */
static inline bool lsmprop_is_set(struct lsm_prop *prop) { return false; }
...
static inline u64 lsm_name_to_attr(const char *name) { return LSM_ATTR_UNDEF; }
...
static inline int early_security_init(void) { return 0; }
...
static inline int security_capset(struct cred *new, const struct cred *old,
                                  const kernel_cap_t *effective,
                                  const kernel_cap_t *inheritable,
                                  const kernel_cap_t *permitted) {
...
static inline void
security_bprm_committing_creds(const struct linux_binprm *bprm) {}

static inline void
security_bprm_committed_creds(const struct linux_binprm *bprm) {}
...
static inline int security_sb_kern_mount(struct super_block *sb) { return 0; }
...
static inline int
security_inode_init_security_anon(struct inode *inode, const struct qstr *name,
                                  const struct inode *context_inode) {
  return 0;
}
...
static inline int security_inode_mkdir(struct inode *dir, struct dentry *dentry,
                                       int mode) {
  return 0;
}
...
static inline int security_file_open(struct file *file) { return 0; }

static inline int security_file_post_open(struct file *file, int mask) {
  return 0;
}

static inline int security_file_truncate(struct file *file) { return 0; }
...
static inline int security_kernel_post_read_file(struct file *file, char *buf,
                                                 loff_t size,
                                                 enum kernel_read_file_id id) {
  return 0;
}
...
static inline int security_shm_alloc(struct kern_ipc_perm *shp) { return 0; }
...
static inline int security_sem_alloc(struct kern_ipc_perm *sma) { return 0; }
...
static inline int security_locked_down(enum lockdown_reason what) { return 0; }
static inline int security_lock_kernel_down(const char *where,
                                            enum lockdown_reason level) {
  return 0;
}
static inline int lsm_fill_user_ctx(struct lsm_ctx __user *uctx, u32 *uctx_len,
                                    void *val, size_t val_len, u64 id,
                                    u64 flags) {
  return -EOPNOTSUPP;
}

static inline int security_bdev_alloc(struct block_device *bdev) { return 0; }
...
#else
static inline int security_watch_key(struct key *key) { return 0; }
#endif
...
static inline int security_socket_post_create(struct socket *sock, int family,
...
static inline int security_socket_getsockopt(struct socket *sock, int level,
                                             int optname) {
  return 0;
}

static inline int security_socket_setsockopt(struct socket *sock, int level,
                                             int optname) {
  return 0;
}
...
static inline int
security_xfrm_state_pol_flow_match(struct xfrm_state *x, struct xfrm_policy *xp,
                                   const struct flowi_common *flic) {
  return 1;
}
...
static inline int security_key_alloc(struct key *key, const struct cred *cred,
                                     unsigned long flags) {
  return 0;
}
...
static inline void
security_key_post_create_or_update(struct key *keyring, struct key *key,
                                   const void *payload, size_t payload_len,
                                   unsigned long flags, bool create) {}
...
securityfs_create_file(const char *name, umode_t mode, struct dentry *parent,
...
static inline struct dentry *
securityfs_create_symlink(const char *name, struct dentry *parent,
                          const char *target,
                          const struct inode_operations *iops) {
  return ERR_PTR(-ENODEV);
}
...
static inline int security_uring_cmd(struct io_uring_cmd *ioucmd) { return 0; }
#endif /* CONFIG_SECURITY */
