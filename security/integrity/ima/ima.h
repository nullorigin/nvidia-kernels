...
#define IMA_DO_MASK                                                            \
  (IMA_MEASURE | IMA_APPRAISE | IMA_AUDIT | IMA_HASH | IMA_APPRAISE_SUBMASK)
#define IMA_DONE_MASK                                                          \
  (IMA_MEASURED | IMA_APPRAISED | IMA_AUDITED | IMA_HASHED | IMA_COLLECTED |   \
...
#define IMA_APPRAISE_SUBMASK                                                   \
  (IMA_FILE_APPRAISE | IMA_MMAP_APPRAISE | IMA_BPRM_APPRAISE |                 \
   IMA_READ_APPRAISE | IMA_CREDS_APPRAISE)
#define IMA_APPRAISED_SUBMASK                                                  \
  (IMA_FILE_APPRAISED | IMA_MMAP_APPRAISED | IMA_BPRM_APPRAISED |              \
   IMA_READ_APPRAISED | IMA_CREDS_APPRAISED)
...
#define __ima_hooks(hook)                                                      \
  hook(NONE, none) hook(FILE_CHECK, file) hook(MMAP_CHECK, mmap)               \
      hook(MMAP_CHECK_REQPROT, mmap_reqprot) hook(BPRM_CHECK, bprm)            \
          hook(CREDS_CHECK, creds) hook(POST_SETATTR, post_setattr)            \
              hook(MODULE_CHECK, module) hook(FIRMWARE_CHECK, firmware)        \
...
static inline bool ima_queue_key(struct key *keyring, const void *payload,
                                 size_t payload_len) {
  return false;
}
...
void ima_store_measurement(struct ima_iint_cache *iint, struct file *file,
                           ...
                           struct ima_template_desc *template_desc);
int process_buffer_measurement(struct mnt_idmap *idmap, struct inode *inode,
                               const void *buf, int size, const char *eventname,
                               enum ima_hooks func, int pcr,
                               const char *func_data, bool buf_hash, u8 *digest,
                               size_t digest_len);
...
int ima_must_appraise(struct mnt_idmap *idmap, struct inode *inode, int mask,
                      enum ima_hooks func);
...
static inline int
ima_appraise_measurement(enum ima_hooks func, struct ima_iint_cache *iint,
                         struct file *file, const unsigned char *filename,
                         struct evm_ima_xattr_data *xattr_value, int xattr_len,
                         const struct modsig *modsig) {
...
static inline int ima_filter_rule_match(struct lsm_prop *prop, u32 field,
                                        u32 op, void *lsmrule) {
  return -EINVAL;
}
